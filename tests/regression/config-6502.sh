#!/bin/bash
# tests/regression/config-6502.sh
#
# Configuration du compilateur de référence pour la famille 6502.
# Chargé automatiquement par run-regression.sh pour les cibles :
#   c64, mega65, x16
#
# Modifier REF_COMPILER pour changer de compilateur de référence.
#
# Valeurs supportées :
#   acme  — ACME assembler (https://sourceforge.net/projects/acme-crossass/)
#   ca65  — ca65 assembler du projet cc65 (https://cc65.github.io/)
#
# Pour ajouter une nouvelle architecture (ex: z80), créer config-z80.sh
# avec les mêmes fonctions : ref_assemble, ref_check_available.
#
    ...
    case "$REF_COMPILER" in
        ...
        acme)
            ...
            case "$target" in
                ...
                x16)
                    ...
                    ;;
                *)
                    echo "config-6502.sh: cible '$target' inconnue pour acme" >&2
                    result=1
                    ;;
            esac
        ...
        ca65)
            ...
            case "$target" in
                ...
                x16)    ca65_cpu="65C02" ;;
                *)
                    echo "config-6502.sh: cible '$target' inconnue pour ca65" >&2
        ...
        *)
            echo "config-6502.sh: compilateur '$REF_COMPILER' inconnu" >&2
            result=1
            ;;
    esac
...
# --------------------------------------------------------------------------
#  Déclaration des cibles gérées par ce fichier de config
#  Utilisé par run-regression.sh pour charger le bon config-*.sh
# --------------------------------------------------------------------------

# config_handles_target TARGET
#   Retourne 0 si ce fichier de config gère la cible, 1 sinon.
config_handles_target() {
    case "$1" in
        c64|mega65|x16) return 0 ;;
        *)              return 1 ;;
    esac
}
