...
if [[ $UPDATE -eq 1 ]]; then
    if ! ref_check_available; then
        echo "Erreur : compilateur de référence '$REF_COMPILER' non trouvé." >&2
        exit 1
    fi

    log "${BLUE}Régénération des binaires de référence ($REF_COMPILER)${RESET}"
    updated=0
    errors=0
...
if ! ref_check_available; then
    log "${YELLOW}Avertissement : $REF_COMPILER non trouvé.${RESET}"
    log "Utilisation des .ref.prg existants uniquement."
fi
